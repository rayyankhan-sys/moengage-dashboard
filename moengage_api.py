    ...
    def _validate_response_shape(self, response_data: Dict, endpoint_type: str, request_id: str) -> bool:
        ...
        try:
            if endpoint_type == "segmentation":
                # Must have "id" or "segment_id"
                if "id" not in response_data and "segment_id" not in response_data:
    ...
    def create_segment(self, segment_payload: Dict[str, Any]) -> Tuple[str, Dict]:
        ...
        for attempt in range(3):
            ...
            try:
                ...
                segment_id = response.get("id") or response.get("segment_id")
                if not segment_id:
                    logger.warning(f"No segment ID in response: {response}")
                    return None, response
